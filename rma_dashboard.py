                    ...
                    for rma in rmas:
                        ...
                        if status_filter == "NoTrack":
                            try:
                                ...
                                if has_tracking: continue 
                            except: continue

                        # 2. Safe Date Logic (The Fix)
                        # We try to get 'createdAt'. If missing, we grab the first event date.
                        raw_date = rma.get('createdAt')
                        if not raw_date:
                            events = rma.get('events', [])
                            if events and len(events) > 0:
                                raw_date = events[0].get('eventDate')
                        
                        # Ensure we have a string before slicing
                        display_date = str(raw_date)[:10] if raw_date else "N/A"
