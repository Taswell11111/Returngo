...
def set_filter(store, status):
    ...
    # Check if we have data for this store, if not, force a sync
    current_count = store_counts.get(store, {}).get(status, 0)
    if current_count == 0 and status != "All" and status not in ["NoTrack", "Flagged"]:
